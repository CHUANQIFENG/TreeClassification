...
class DataLoader(object):
    ...
    def next_train_batch(self):
        ...
        for sub_cursor in range(self.cursor, self.cursor + self.batch_size):
            ...
            image_path_nDSM = self.dir_nDSM + '/CHM' + item_name.replace('.png','.mat')

            # For RGB file
            array_RGB = np.array(Image.open(image_path_RGB))

            mat_nDSM = scipy.io.loadmat(image_path_nDSM)['image_sub']
            array_nDSM=np.asarray(mat_nDSM)
            ...
            # Augmentation
            #array_RGB,array_nDSM = self.augmentations(array_RGB,array_nDSM)

            array_RGB = array_RGB.transpose((2, 0, 1))
            array_nDSM = np.expand_dims(array_nDSM, axis=0)
            array_label = np.expand_dims(array_label, axis=0)

            self.imgs_RGB[index,...] = array_RGB
            self.imgs_nDSM[index,...] = array_nDSM
            self.labels[index, ...] = array_label

            index=index+1

        self.cursor += self.batch_size
        return self.imgs_RGB,self.imgs_nDSM, self.labels
