...
class MyDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        top[1].reshape(self.batch_size, 1, self.target_size_w, self.target_size_h)  # CHM
    ...
    def forward(self, bottom, top):
       if self.stage == 'train': 
            imgs_RGB,imgs_nDSM, labels = self.batch_loader.next_train_batch()
            top[0].data[...] = imgs_RGB
            top[1].data[...] = imgs_nDSM
            top[2].data[...] = labels
       else:
            imgs_RGB,imgs_nDSM, labels= self.batch_loader.next_test_batch()
            top[0].data[...] = imgs_RGB
            top[1].data[...] = imgs_nDSM
            top[2].data[...] = labels
