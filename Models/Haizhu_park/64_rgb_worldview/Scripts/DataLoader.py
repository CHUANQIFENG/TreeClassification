...
class DataLoader(object):
    ...
    def next_train_batch(self):
        ...
        for sub_cursor in range(self.cursor, self.cursor + self.batch_size):
            ...
            image_path_WV = self.dir_WV + '/WV' + item_name

            # For RGB file
            array_RGB = np.array(Image.open(image_path_RGB))
            array_WV = np.array(Image.open(image_path_WV))
            ...
            # Augmentation
            array_RGB,array_WV = self.augmentations(array_RGB,array_WV)

            array_RGB = array_RGB.transpose((2, 0, 1))
            array_WV = array_WV.transpose((2, 0, 1))
            array_label = np.expand_dims(array_label, axis=0)

            self.imgs_RGB[index,...] = array_RGB
            self.imgs_WV[index,...] = array_WV
            self.labels[index, ...] = array_label

            index=index+1

        self.cursor += self.batch_size
        return self.imgs_RGB,self.imgs_WV, self.labels
