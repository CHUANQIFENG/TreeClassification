...
sys.path.append('/home/lucas/caffe-new/python')
import caffe
...
class MyDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        #if self.stage == 'train': 
        top[1].reshape(self.batch_size, 1, 1, 1)  # Label
    ...
    def forward(self, bottom, top):
       if self.stage == 'train': 
            imgs_RGB, labels = self.batch_loader.next_train_batch()
            top[0].data[...] = imgs_RGB
            top[1].data[...] = labels
       else:
            imgs_RGB, labels= self.batch_loader.next_test_batch()
            top[0].data[...] = imgs_RGB
            top[1].data[...] = labels
